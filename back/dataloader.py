...
#データの場所
xml_paths_train = glob("dataset/train/Annotations/*.xml")
xml_paths_val = glob("dataset/val/Annotations/*.xml")

image_dir_train = "dataset/train/JPEGImages"
image_dir_val = "dataset/val/JPEGImages"
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #######デバック処理（int変換）
        boxes = torch.tensor(records[["xmin", "ymin", "xmax", "ymax"]].values,
                             dtype=torch.float32)
...
def dataloader(xml_paths_train, xml_paths_val, image_dir_train, image_dir_val):
    ...
    #bdd100kval
    # classes = [
    #     'person', 'traffic light', 'train', 'traffic sign', 'rider', 'car',
    #     'bike', 'motor', 'truck', 'bus'
    # ]
    ...
    for path in xml_paths:
        ...
        for bbox in bboxs:
            ...
            tmp["image_id"] = image_id
            df_val = pd.concat([df_val, tmp], ignore_index=True)        
